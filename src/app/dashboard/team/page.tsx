...
export default function TeamPage() {
  ...
  // useCallback + a functional setState, so the effect below can depend on
  // this honestly. It previously read `appId` from the render closure it was
  // created in and ran with an empty dependency array — which happens to be
  // harmless while it only runs on mount, but silently reads a stale value
  // the moment anything makes it run again.
  const loadApps = useCallback(async () => {
    const [r, sr] = await Promise.all([fetch("/api/apps"), fetch("/api/team/apps")]);
    const d = await r.json(); const sd = await sr.json();
    const list: AppItem[] = (d.apps ?? []).map((a: AppItem) => ({ id: a.id, name: a.name }));
    const shared = sd.apps ?? [];
    setApps(list); setSharedApps(shared);
    setAppId((current) => current || list[0]?.id || shared[0]?.id || "");
  }, []);
  ...
  useEffect(() => { loadApps().finally(() => setLoading(false)); }, [loadApps]);
